...
int buttonState (unsigned int* gpio, int buttonNum){
	...
	int pin_state = level_reg & (1 << buttonNum);
	usleep (1000);

	return pin_state;
}

void startProg (int pinNum, unsigned int*gpio, int buttonNum){
	int buttonp = 1; // loop condition
	int pushed = 0; //counts when button is pushed
	turnOn(pinNum,gpio); // turn on first light
	while (buttonp){		
		
			if(!buttonState(gpio, buttonNum)) {//pinstate is false when button is pushed
				pushed = 1;
				}
			if(buttonState(gpio, buttonNum) && pushed){
				buttonp = 0;
			}
		}
	turnOff(pinNum,gpio); // turn off to show program has started
}

void numLightBlinks (int i, const int size, int switchState[], unsigned int*gpio, int buttonNum){
	while (i<size){
			/*sleep (1000);*///usleep for 1000 ms = 0.001s * 10000 = 10s total
			int button = buttonState(gpio, buttonNum);
...
void blinkLight(int i, int pinNum, int size,int switchState[], unsigned int*gpio){ // I NEED LIGHT TO TURN ON for the number of 1's in ARRAY
	while (i < size-3){
...
int main(void)
{
	...
	setOutput(gp, pin1, pin2);
	
	while (1){
		startProg(pin1, gp, buttonPin);//once pin_state is false (button is pressed) --> Turn off light 1
		usleep(500000);
		numLightBlinks(i, sz, switchState, gp, buttonPin);
		blinkLight(i, pin2, sz, switchState, gp);	
		usleep(5000000);
	}
